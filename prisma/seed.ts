...
async function main() {
  ...
  for (const b of BUILDING_NAMES) {
    ...
    for (let f = 1; f <= Math.min(b.floors, 5); f++) {
      ...
      for (let u = 1; u <= Math.min(unitsPerFloor, 4); u++) {
        ...
        const rent = Math.floor(40000 + Math.random() * 160000);
        await prisma.unit.upsert({
          ...
          create: {
            ...
            carpetArea: 800 + Math.random() * 1200,
            builtUpArea: 1000 + Math.random() * 1500,
            superBuiltUpArea: 1200 + Math.random() * 1800,
            monthlyRent: rent,
  ...
  for (const t of TASKS) {
    await prisma.task.create({
      data: {
        ...
        status: t.status,
        priority: t.priority,
        buildingId: buildingIds[Math.floor(Math.random() * buildingIds.length)],
  ...
  for (const sv of SITE_VISITS) {
    await prisma.siteVisit.create({
      data: {
        clientId: clientIds[Math.floor(Math.random() * clientIds.length)],
        buildingId: buildingIds[Math.floor(Math.random() * buildingIds.length)],
        scheduledAt: new Date(Date.now() + Math.floor(Math.random() * 7) * 86400000),
        status: sv.status,
        notes: sv.notes,
        assignedTo: workerUser.id,
        createdBy: adminUser.id,
