...
export class SmsService {
  ...
  async send(message: SmsMessage): Promise<{ success: boolean; messageId?: string }> {
    ...
    this.logger.warn(`Unknown SMS provider: ${this.provider}, falling back to console`);
    this.logger.log(`[SMS:${message.to}] ${message.message}`);
    return { success: true, messageId: `console-${Date.now()}` };
  }

  private async sendTwilio(message: SmsMessage): Promise<{ success: boolean; messageId?: string }> {
    try {
      ...
      const result = await response.json() as { sid?: string; error_code?: string; error_message?: string };

      if (!response.ok) {
        this.logger.error(`Twilio SMS failed: ${result.error_message || result.error_code}`);
  ...
  private async sendTextlocal(message: SmsMessage): Promise<{ success: boolean; messageId?: string }> {
    try {
      ...
      const result = await response.json() as { status?: string; message?: { id?: string } };
      ...
      this.logger.log(`Textlocal SMS sent to ${message.to}: ${result.message?.id}`);
