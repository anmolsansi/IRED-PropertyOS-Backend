...
export class PushService {
  ...
  async send(message: PushMessage): Promise<{ success: boolean; messageId?: string }> {
    ...
    this.logger.warn(`Unknown push provider: ${this.provider}, falling back to console`);
    this.logger.log(`[PUSH:${message.to}] ${message.title}: ${message.body}`);
    return { success: true, messageId: `console-${Date.now()}` };
  }

  private async sendFcm(message: PushMessage): Promise<{ success: boolean; messageId?: string }> {
    try {
      ...
      const result = await response.json() as { success?: number; message_id?: string; results?: Array<{ message_id?: string; error?: string }> };
