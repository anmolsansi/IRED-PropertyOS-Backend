...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    jwtService = {
      ...
      verify: jest.fn().mockReturnValue({ sub: 'user-1', email: 'test@test.com', jti: 'jti-1', exp: 9999999999 }),
    };
  ...
  describe('login', () => {
    ...
    it('should throw if password invalid', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue({ id: '1', status: 'active', passwordHash: hash, email: 'test@test.com' });
      await expect(service.login('test@test.com', 'wrong')).rejects.toThrow('Invalid email or password');
    });

    it('should return userId and requiresOtp on success', async () => {
      const hash = await argon2.hash('correct-password');
      prisma.user.findUnique.mockResolvedValue({
        id: 'user-1', status: 'active', passwordHash: hash, email: 'test@test.com',
      });
      const result = await service.login('test@test.com', 'correct-password');
      expect(result).toHaveProperty('userId', 'user-1');
      expect(result).toHaveProperty('requiresOtp', true);
      expect(otpService.generate).toHaveBeenCalledWith('user-1', 'EMAIL_VERIFICATION');
