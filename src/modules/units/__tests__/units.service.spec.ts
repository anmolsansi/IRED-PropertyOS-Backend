...
describe('UnitsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      ...
      changeRequest: { create: jest.fn().mockResolvedValue({ id: 'cr-1', changeItems: [] }) },
    };
  ...
  describe('create', () => {
    it('should create a unit', async () => {
      const result = await service.create({ unitNumber: '101', buildingId: 'b-1' }, 'user-1');
  ...
  describe('update', () => {
    it('should update directly for admin', async () => {
      prisma.unit.findUnique.mockResolvedValue({ id: 'u-1', unitNumber: '101' });
      const result = await service.update('u-1', { unitNumber: '102' }, 'admin-1', true);
    ...
    it('should throw NotFoundException for non-existent unit', async () => {
      ...
      await expect(service.update('u-1', {}, 'user-1', true)).rejects.toThrow('Unit not found');
    });
  });
