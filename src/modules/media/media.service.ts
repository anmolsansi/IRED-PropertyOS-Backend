...
import { FileType, UploadStatus } from '../../generated/prisma';
...
export class MediaService {
  ...
  constructor(
    ...
    this.bucket = this.config.get<string>('app.s3.bucket') || 'propertyos-media';
  ...
  async completeUpload(id: string, fileSizeBytes?: number) {
    ...
    return this.prisma.media.update({
      ...
      data: {
        ...
        uploadedAt: new Date(),
        fileSizeBytes: fileSizeBytes ? BigInt(fileSizeBytes) : media.fileSizeBytes,
      },
    });
  }
  ...
  async updateMetadata(id: string, data: { documentCategoryId?: string; notes?: string }) {
    const media = await this.prisma.media.findUnique({ where: { id } });
    if (!media) throw new NotFoundException('Media not found');
