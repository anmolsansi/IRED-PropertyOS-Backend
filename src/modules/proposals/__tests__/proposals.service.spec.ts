...
describe('ProposalsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      ...
      proposal: {
        create: jest.fn(),
        findMany: jest.fn().mockResolvedValue({ data: [], meta: { total: 0, page: 1, limit: 20, totalPages: 0 } }),
        findUnique: jest.fn(),
  ...
  describe('create', () => {
    it('should create a proposal in the database', async () => {
      prisma.client.findUnique.mockResolvedValue({ id: 'cl-1', name: 'Test Client' });
      prisma.unit.findMany.mockResolvedValue([{ id: 'u-1', unitNumber: '101' }]);
  ...
  describe('findOne', () => {
    it('should return a proposal by id', async () => {
      prisma.proposal.findUnique.mockResolvedValue({ id: 'prop-1', client: {} });
    ...
    it('should throw if not found', async () => {
      ...
      await expect(service.findOne('missing')).rejects.toThrow('Proposal not found');
    });
  });
