...
describe('DealsService', () => {
  ...
  describe('update', () => {
    it('should throw if not found', async () => {
      ...
      await expect(service.update('nonexistent', { status: 'closed' }, 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('should throw on invalid status transition', async () => {
      prisma.deal.findUnique.mockResolvedValue({ id: 'd-1', status: 'negotiation' });
      await expect(service.update('d-1', { status: 'shortlisted' }, 'user-1')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should set closedAt when closing', async () => {
      prisma.deal.findUnique.mockResolvedValue({ id: 'd-1', status: 'agreement_shared' });
