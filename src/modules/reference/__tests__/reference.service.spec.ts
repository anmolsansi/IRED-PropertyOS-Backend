...
describe('ReferenceService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      state: { findMany: jest.fn().mockResolvedValue([{ id: 's-1', name: 'Delhi' }]) },
      city: { findMany: jest.fn().mockResolvedValue([{ id: 'c-1', name: 'New Delhi' }]) },
      locality: { findMany: jest.fn().mockResolvedValue([{ id: 'l-1', name: 'Connaught Place' }]) },
