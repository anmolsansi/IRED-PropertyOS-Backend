...
describe('ContactsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      contact: {
        findUnique: jest.fn(),
        create: jest.fn().mockResolvedValue({ id: 'c-1', fullName: 'Test Contact' }),
  ...
  describe('update', () => {
    it('should update directly for admin', async () => {
      prisma.contact.findUnique.mockResolvedValue({ id: 'c-1', fullName: 'Original' });
      const result = await service.update('c-1', { fullName: 'Updated' }, 'admin-1', true);
      expect(result).toHaveProperty('fullName', 'Updated');
    });

    it('should throw NotFoundException for non-existent contact', async () => {
      prisma.contact.findUnique.mockResolvedValue(null);
      await expect(service.update('c-1', {}, 'user-1', true)).rejects.toThrow('Contact not found');
    });
  });
