...
describe('SiteVisitsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      siteVisit: {
        ...
        create: jest.fn().mockResolvedValue({ id: 'sv-1', status: 'scheduled' }),
        update: jest.fn().mockResolvedValue({ id: 'sv-1', status: 'completed' }),
      },
    };
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException', async () => {
      ...
      await expect(service.findOne('sv-1')).rejects.toThrow('Site visit not found');
    });
  });

  describe('create', () => {
    it('should create a site visit', async () => {
      const result = await service.create({ scheduledAt: new Date() }, 'user-1');
  ...
  describe('update', () => {
    it('should update a site visit', async () => {
      prisma.siteVisit.findUnique.mockResolvedValue({ id: 'sv-1', status: 'scheduled' });
      const result = await service.update('sv-1', { status: 'confirmed' });
      expect(result).toHaveProperty('status', 'completed');
    });

    it('should throw on invalid status transition', async () => {
      prisma.siteVisit.findUnique.mockResolvedValue({ id: 'sv-1', status: 'completed' });
      await expect(service.update('sv-1', { status: 'scheduled' })).rejects.toThrow(BadRequestException);
    });
  });

  describe('complete', () => {
    it('should mark as completed', async () => {
      prisma.siteVisit.findUnique.mockResolvedValue({ id: 'sv-1', status: 'confirmed' });
