...
describe('BuildingsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      building: {
        ...
        create: jest.fn().mockResolvedValue({ id: 'b-1', name: 'Test Building' }),
  ...
  describe('update', () => {
    it('should update directly for admin', async () => {
      prisma.building.findUnique.mockResolvedValue({ id: 'b-1', name: 'Original' });
      const result = await service.update('b-1', { name: 'Updated' }, 'admin-1', true);
      expect(result).toHaveProperty('name', 'Updated');
    });

    it('should create change request for worker', async () => {
      prisma.building.findUnique.mockResolvedValue({ id: 'b-1', name: 'Original', stateId: 's1', cityId: 'c1' });
      const result = await service.update('b-1', { name: 'Worker Edit' }, 'worker-1', false);
