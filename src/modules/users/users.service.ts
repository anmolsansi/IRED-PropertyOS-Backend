...
export class UsersService {
    ...
    if (data.stateIds || data.cityIds) {
      ...
      if (data.stateIds) {
        for (const stateId of data.stateIds) {
          assignments.push({ userId: user.id, stateId, assignmentType: 'state' });
        }
      }
      if (data.cityIds) {
      ...
      await this.prisma.workerGeographicAssignment.createMany({ data: assignments });
    }
  ...
  async assignGeographicScope(userId: string, assignments: {
    ...
    await this.prisma.workerGeographicAssignment.createMany({ data: newAssignments });
  ...
  async reassignUnits(fromWorkerId: string, toWorkerId: string) {
    const fromWorker = await this.prisma.user.findUnique({ where: { id: fromWorkerId } });
    const toWorker = await this.prisma.user.findUnique({ where: { id: toWorkerId } });
