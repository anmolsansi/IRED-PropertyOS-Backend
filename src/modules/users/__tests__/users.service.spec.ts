...
describe('UsersService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      user: {
        ...
        create: jest.fn().mockResolvedValue({ id: 'u-1', email: 'test@test.com', role: 'WORKER' }),
  ...
  describe('invite', () => {
    ...
    it('should throw if email already exists', async () => {
      ...
      await expect(
        service.invite({ email: 'existing@test.com', fullName: 'Test', role: 'WORKER' }),
  ...
  describe('updateStatus', () => {
    ...
    it('should throw if user not found', async () => {
      prisma.user.findUnique.mockResolvedValue(null);
      await expect(service.updateStatus('u-1', 'active')).rejects.toThrow('User not found');
    });
  });
  ...
  describe('reassignUnits', () => {
    ...
    it('should throw if source worker not found', async () => {
      prisma.user.findUnique.mockResolvedValue(null);
      await expect(service.reassignUnits('from-1', 'to-1')).rejects.toThrow('Source worker not found');
    });
  });
  ...
  describe('resetPassword', () => {
    ...
    it('should throw if user not found', async () => {
      prisma.user.findUnique.mockResolvedValue(null);
      await expect(service.resetPassword('u-1')).rejects.toThrow('User not found');
    });
  });
});
